 ...
 <#if PAGE_DATA??>
     ...
     <#if PAGE_DATA.totalPages gt 0>
         <#list 1..PAGE_DATA.totalPages as page>
             <#if page <= 3 || (PAGE_DATA.totalPages-page) < 3 || (PAGE_DATA.number+1-page)?abs<3 >
                 ...
                 <#else>
                     <input onclick="javascript:page(${mid},${catId },'btnPage','${page-1}')" class="" type="button" value="${page}" /> 
                 </#if>
                 <#assign continueEnter=false>
             <#else>
                 <#if !continueEnter>
     ...
     <#else>
         <input onclick="javascript:page(${mid},${catId },'btnPage','${PAGE_DATA.number-1}')" class="block" type="button" value="上一页" />                
     </#if>
     ...
     <#else>
         <input onclick="javascript:page(${mid},${catId },'btnPage','${PAGE_DATA.number+1}')" class="block" type="button" value="下一页" /> 
     </#if>
 </#if>

    <input id="jump-page" class="page_text" type="" value="${PAGE_DATA.number+1 }" />
    <input class= "block" onclick="javascript:pageJump(${mid},${catId },'btnPage')" type="button" value="确定" />
